...
def get_ner_fmeasure(golden_lists, predict_lists, label_type="BMES"):
    ...
    entity_types = ["PER", "LOC", "ORG", "OTHER", "all"]
    for e_type in entity_types:
        if e_type == "all":
            e_right_full = right_full
            e_golden_full = golden_full
            e_predict_full = predict_full
        else:
            e_right_full = [e for e in right_full if e_type in e]
            e_golden_full = [e for e in golden_full if e_type in e]
            e_predict_full = [e for e in predict_full if e_type in e]

        right_num = len(e_right_full)
        golden_num = len(e_golden_full)
        predict_num = len(e_predict_full)
        ...
        if label_type.upper().startswith("B"):
            print("Entity type: ", e_type, "p = ", precision, " r = ", recall, " f = ", f_measure)
        else:
            print("Entity type: ", e_type, "Right token = ", right_tag, " All token = ", all_tag, " acc = ", accuracy)
