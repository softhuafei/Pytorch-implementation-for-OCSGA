...
class Data:
    def __init__(self):
        ...
        self.norm_object_emb = False
        self.word_alphabet = Alphabet('word')
        self.char_alphabet = Alphabet('character')
        self.object_alphabet = Alphabet('object')
        ...
        self.pretrain_object_embedding = None
    ...
    def show_data_summary(self):
        ...
        print("     MAX      LENGTH: %s" % (self.MAX_OBJECT_NB))
        ...
        print("     topic embedding size: %s" % (self.object_emb_dim))
        ...
        print("     object    file directory: %s" % (self.object_dir))
        ...
        print("     Model         n_gram: %s"%(self.n_gram))
        if self.use_char:
        ...
        print("     Hyper      multi_head: %s"%(self.HP_multi_head))
        print("     Hyper       SGA layer: %s"%(self.HP_SGA_layer))
        print("     Hyper         dropout: %s"%(self.HP_dropout))
        print("     Hyper     MCA dropout: %s"%(self.HP_MCA_dropout))
    ...
    def build_object_alphabet(self, input_file):
        in_lines = open(input_file, 'r', encoding='utf-8').readlines()
        for line in in_lines:
            for topic in line.strip().split(' ')[1:]:
                self.object_alphabet.add(topic)
        self.object_alphabet_size = self.object_alphabet.size()
    ...
    def build_pretrain_emb(self):
        ...
        if self.object_emb_dir:
            print("Load pretrained object embedding, norm: %s, dir: %s" % (self.norm_object_emb, self.object_emb_dir))
            self.pretrain_object_embedding, self.object_emb_dim = build_pretrain_embedding(self.object_emb_dir, self.object_alphabet, self.object_emb_dim, self.norm_object_emb)

    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, object_file=self.object_dir, object_alphabet=self.object_alphabet)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, object_file=self.object_dir, object_alphabet=self.object_alphabet)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, object_file=self.object_dir, object_alphabet=self.object_alphabet)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, object_file=self.object_dir, object_alphabet=self.object_alphabet)
        else:
    ...
    def read_config(self,config_file):
        ...
        the_item = 'object_dir'
        if the_item in config:
            self.object_dir = config[the_item]
        the_item = 'decode_dir'
        if the_item in config:
        ...
        the_item = 'object_emb_dir'
        if the_item in config:
            self.object_emb_dir = config[the_item]
        ...
        the_item = 'MAX_OBJECT_NB'
        if the_item in config:
            self.MAX_OBJECT_NB = int(config[the_item])
        ...
        the_item = 'object_emb_dim'
        if the_item in config:
            self.object_emb_dim = int(config[the_item])
        ...
        the_item = 'multi_head'
        if the_item in config:
            self.HP_multi_head = int(config[the_item])

        the_item = 'SGA_layer'
        if the_item in config:
            self.HP_SGA_layer = int(config[the_item])

        the_item = 'dropout'
        if the_item in config:
            self.HP_dropout = float(config[the_item])

        the_item = 'MCA_dropout'
        if the_item in config:
            self.HP_MCA_dropout = float(config[the_item])
        the_item = 'lstm_layer'
        if the_item in config:
        ...
        the_item ='n_gram'
        if the_item in config:
            self.n_gram = int(config[the_item])
