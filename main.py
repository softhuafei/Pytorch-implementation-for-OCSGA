...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask, batch_obj, obj_mask = batchify_with_label(instance, data.HP_gpu, False, data.sentence_classification, max_object_nb=data.MAX_OBJECT_NB)
...
def batchify_sequence_labeling_with_label(input_batch_list, gpu, if_train=True, max_object_nb=0):
    ...
    for idx, (obj, objlen) in enumerate(zip(objects, object_lengths)):
        objlen = objlen.item()
        if objlen < max_object_nb:
            object_tensor[idx, :objlen] = torch.LongTensor(obj)
            object_mask[idx,:objlen] = torch.Tensor([1]*objlen)
        else:
            object_tensor[idx, :max_object_nb] = torch.LongTensor(obj[:max_object_nb])
            object_mask[idx, :max_object_nb] = torch.Tensor([1] * max_object_nb)
    ...
    object_tensor = object_tensor[word_perm_idx]
    object_mask = object_mask[word_perm_idx]
    ### deal with char
    ...
    char_seq_tensor = char_seq_tensor[word_perm_idx].view(batch_size*max_seq_len,-1) # 将word当成一个sample
    ...
    if gpu:
        ...
        mask = mask.cuda()
    return word_seq_tensor,feature_seq_tensors, word_seq_lengths, word_seq_recover, char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, mask, object_tensor, object_mask
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        right_token = 0
        whole_token = 0
        test_best_f, test_best_iter = 0, 0
        dev_best_f, dev_best_f = 0, 0
